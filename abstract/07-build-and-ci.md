...
```bash
npm install
npm run dev      # build, watch, and serve with live reload on :4000
...
`npm run dev` proxies Ghost through browser-sync rather than serving files
itself — a static server has no Handlebars, no Members and no `{{ghost_head}}`,
which is most of what there is to look at. CSS is **injected** without a
reload, so a style change keeps your scroll position and any open menu;
templates and scripts reload the page, because they cannot be injected. Set
`GHOST_URL` if Ghost is not on `localhost:2369`.

**Why the output is always named `.min`.** Handlebars cannot see `NODE_ENV`,
so `default.hbs` has to name one file. Two names would mean a runtime switch
the platform does not offer, or a template edit at deploy time — and a
template edit that only happens at deploy time is one that gets forgotten. So
the name is constant and the contents change.
...
| `css` | `screen.css` → `built/screen.min.css` | Tailwind v4 through `@tailwindcss/postcss`; cssnano in production only |
| `js` | `assets/js/0-vendor/*.js` → `built/main.min.js` | concatenated in a fixed order, then terser in production |
| `checkLayers` | the cascade contract, **on the output** | the source cannot prove it — see [`04`](04-css.md) §2 |
...
which is the white-flash-on-every-navigation bug ([`08`](08-ghost.md)).
