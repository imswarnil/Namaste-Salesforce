...
├── nsds/                   ← VENDORED. Never edit.
│   ├── nsds.css            tokens + the whole .ns-* component layer
│   └── tailwind.css        the @theme token bridge
└── theme/                  ← everything the theme itself owns
    ├── 0-foundation/       theme-local custom properties. ⚠ NOT design tokens
    ├── 1-base/             reset.css, shell.css
    ├── 2-layouts/          page structures the theme owns (empty)
    ├── 3-components/       navbar, logo-sting, pace, typography
    ├── 4-ghost/            koenig.css — Ghost's .kg-* vocabulary
    ├── 5-utilities/        theme utilities (empty, and should stay so)
    └── 9-generated/        icon-bridge.css — machine-written
```

**The digits are the cascade.** Folder order is import order is precedence
inside `@layer components`: 5 beats 3 beats 1. `theme/README.md` is the map,
and `0-foundation/README.md` explains at length why that folder holds no
design tokens — NSDS owns those, and a second copy is a second source of
truth for a value this theme and the LMS must agree on.

`assets/built/screen.min.css` is what `default.hbs` links; the name is
constant while the contents differ between dev and production, because
Handlebars cannot see `NODE_ENV`. `gulpfile.mjs` explains the trade.
...
most of the rework in [`10`](09-lessons.md) is just this list.
