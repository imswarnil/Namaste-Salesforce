...
| routes.yaml filter | `tag:hash-training-module` | **→** |
...
*What does someone see at `/training/`? A grid of cards, a dense table, a
...
| | |
|---|---|
| listing archetype | `training-index.html` |
| single archetype | `training-module.html`, `training-post.html` |
| components it needs | **→** |
| behaviour script | `training.js`, `toc.js` |
...
- [ ] listing template ``training.hbs`` exists and is named in `routes.yaml`
