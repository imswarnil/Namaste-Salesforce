...
| internal tag | `— (no tag: whatever matches nothing else)` | **→** |
| routes.yaml filter | `— (unfiltered, LAST)` | **→** |
...
*What does someone see at `/archive/`? A grid of cards, a dense table, a
...
| | |
|---|---|
| listing archetype | `blog-listing.html` |
| single archetype | `blog-post.html` |
| components it needs | **→** |
| behaviour script | — |
...
- [ ] listing template ``index.hbs`` exists and is named in `routes.yaml`
