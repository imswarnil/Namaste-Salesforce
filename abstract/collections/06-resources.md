...
| | proposed | decided |
|---|---|---|
| mount path | `/resources/` | **→** |
| permalink | `/resources/{slug}/` | **→** |
| internal tag | `#resource` | **→** |
| routes.yaml filter | `tag:hash-resource` | **→** |
...
*What does someone see at `/resources/`? A grid of cards, a dense table, a
...
| | |
|---|---|
| listing archetype | — (compose; `.ns-card` grid or `.ns-table`) |
...
- [ ] listing template ``resources.hbs`` exists and is named in `routes.yaml`
