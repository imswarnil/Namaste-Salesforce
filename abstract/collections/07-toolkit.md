...
| routes.yaml filter | `tag:hash-product` | **→** |
...
*What does someone see at `/toolkit/`? A grid of cards, a dense table, a
...
| | |
|---|---|
| listing archetype | — (compose; `.ns-card`) |
| single archetype | `.ns-buybox`, `.ns-product` |
| components it needs | **→** |
| behaviour script | — |
...
- [ ] listing template ``toolkit.hbs`` exists and is named in `routes.yaml`
