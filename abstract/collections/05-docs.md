...
| routes.yaml filter | `tag:hash-docs-section` | **→** |
...
*What does someone see at `/docs/`? A grid of cards, a dense table, a
...
| | |
|---|---|
| listing archetype | — (compose from `.ns-sidenav` + `.ns-tree`) |
| single archetype | — (compose from `.ns-sidenav` + `.ns-post__rail`) |
| components it needs | **→** |
| behaviour script | `rail.js`, `toc.js`, `code.js` |
...
- [ ] listing template ``docs.hbs`` exists and is named in `routes.yaml`
