...
| internal tag | `#course` | **→** |
| routes.yaml filter | `tag:hash-course` | **→** |
...
*What does someone see at `/courses/`? A grid of cards, a dense table, a
...
| | |
|---|---|
| listing archetype | `course-listing.html` |
| single archetype | `course-detail.html`, then `course-player-article.html` for a lesson |
| components it needs | **→** |
| behaviour script | `lms.js`, `code.js` |
...
- [ ] listing template ``courses.hbs`` exists and is named in `routes.yaml`
