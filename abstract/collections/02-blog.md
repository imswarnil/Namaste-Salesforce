...
| permalink | `/blog/{slug}/` | **→** |
| internal tag | `#blog` | **→** |
| routes.yaml filter | `tag:hash-blog` | **→** |
...
*What does someone see at `/blog/`? A grid of cards, a dense table, a
...
| components it needs | **→** |
| behaviour script | `toc.js`, `code.js` |
...
- [ ] listing template ``blog.hbs`` exists and is named in `routes.yaml`
