 ...
 * What lands where:
 *
 *   assets/css/nsds/styles.css        entry — imports everything below
 *   assets/css/nsds/tokens/*.css      colors, spacing, layout, fonts, type, effects
 *   assets/css/nsds/icons/*.css       phosphor.css + icons-gap.css
 *   assets/css/nsds/patterns/*.css    patterns.css
 *   assets/css/nsds/components/css/   the .ns-* class layer shared with the app
 ...
 *   assets/js/nsds/*.js                the system's own BEHAVIOUR
 *
 ...
 *
 * ⚠️ THE JS IS PART OF THE CONTRACT, not an optional extra. Several system
 * components are inert markup without it — .ns-topnav's menus and mobile
 * sheet, the theme switch, .ns-toc's scroll-spy, the lesson rail. Vendoring
 * the CSS but hand-rolling the behaviour is how the two products drift in the
 * half nobody diffs. assets/js/nsds/ is GENERATED; never hand-edit it.
 *
 * theme-init.js is the exception to "load it deferred": it must be INLINED,
 * blocking, in <head> before any stylesheet, or dark-mode readers get a white
 * flash on every navigation. default.hbs does that.
 *
 ...
 *
 * ⚠️ Everything under assets/css/nsds/ is GENERATED. Never hand-edit it — the
...
/**
 * Copied stylesheets sit at assets/css/nsds/<dir>/file.css. To reach
...
async function build(cssTarget, fontTarget, iconTarget, jsTarget, partialTarget) {
  ...
  // The system's runtime. Lands under assets/js/nsds/ so gulp's `js` task —
  // which concatenates assets/js/*.js into built/casper.js — does NOT sweep it
  // up: these are independent modules loaded on their own <script defer>, and
  // concatenating them into the theme bundle would run them in the wrong order
  // and defeat per-page loading.
  await rm(jsTarget, { recursive: true, force: true })
  await mkdir(jsTarget, { recursive: true })
  const jsFrom = join(source, 'assets/js')
  if (existsSync(jsFrom)) {
    for (const entry of await readdir(jsFrom, { withFileTypes: true })) {
      if (entry.isFile() && entry.name.endsWith('.js')) {
        await cp(join(jsFrom, entry.name), join(jsTarget, entry.name))
      }
    }
  }

  // theme-init.js has to be INLINE and BLOCKING in <head> — a <script src> is
  // fetched async and paints too late, which is the white-flash-on-every-
  // navigation bug every design system with dark mode ships once.
  //
  // Pasting its contents into default.hbs would work and then rot: the next
  // upstream fix would live in assets/js/nsds/theme-init.js while the bytes
  // actually running came from a copy nobody re-pasted. So the sync GENERATES
  // the partial, and default.hbs just includes it.
  const themeInit = join(jsTarget, 'theme-init.js')
  if (existsSync(themeInit)) {
    // The file opens with a long /* … */ banner explaining where it must be
    // inlined — and that banner quotes `{{ghost_head}}`. Handlebars parses the
    // partial, sees a mustache inside a comment it does not understand as a
    // comment, and the theme fails gscan with "must contain valid Handlebars".
    // The banner is documentation for the design-system repo, not something
    // that needs shipping in every page's <head>, so it comes off here.
    const raw = await readFile(themeInit, 'utf8')
    const body = raw.replace(/^\s*\/\*[\s\S]*?\*\/\s*/, '')
    await mkdir(join(partialTarget, 'nsds'), { recursive: true })
    await writeFile(
      join(partialTarget, 'nsds/theme-init.hbs'),
      '{{!-- GENERATED by scripts/sync-design.mjs from NS-Design-System/assets/js/\n'
        + '     theme-init.js — do not edit; run `yarn design:sync`.\n\n'
        + '     Inlined and BLOCKING in <head> on purpose. It sets data-theme on\n'
        + '     <html> before first paint, which is the only way to stop a dark-mode\n'
        + '     reader seeing a white flash on every navigation. It shares the\n'
        + '     `ns-theme` storage key with the Next.js app, so a reader crossing\n'
        + '     between the two products keeps their theme. --}}\n'
        + '<script>\n' + body + '</script>\n',
    )
  }
...
if (!existsSync(source)) {
  ...
  if (existsSync(join(themeRoot, 'assets/css/nsds/styles.css'))) {
...
const cssTarget = join(themeRoot, 'assets/css/nsds')
const fontTarget = join(themeRoot, 'assets/fonts')
const iconTarget = join(themeRoot, 'assets/icons')
const jsTarget = join(themeRoot, 'assets/js/nsds')
const partialTarget = join(themeRoot, 'partials')
...
if (checkOnly) {
  ...
  try {
    await build(join(scratch, 'css'), join(scratch, 'fonts'), join(scratch, 'icons'), join(scratch, 'js'), join(scratch, 'partials'))
...
const { cssFiles } = await build(cssTarget, fontTarget, iconTarget, jsTarget, partialTarget)
console.log('✓ synced NS-Design-System → assets/css/nsds')
console.log(`  ${cssFiles} stylesheets, fonts → assets/fonts, sprite → assets/icons, runtime → assets/js/nsds`)
