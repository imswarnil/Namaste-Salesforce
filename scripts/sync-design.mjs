 ...
 *   assets/js/ds/*.js                the system's own BEHAVIOUR
 *
 ...
 * half nobody diffs. assets/js/ds/ is GENERATED; never hand-edit it.
 *
...
async function build(cssTarget, fontTarget, iconTarget, jsTarget, partialTarget) {
  ...
  // The system's runtime. Lands under assets/js/ds/ so gulp's `js` task —
  ...
  // upstream fix would live in assets/js/ds/theme-init.js while the bytes
  ...
  if (existsSync(themeInit)) {
    ...
    await mkdir(join(partialTarget, 'ds'), { recursive: true })
    await writeFile(
      join(partialTarget, 'ds/theme-init.hbs'),
...
const jsTarget = join(themeRoot, 'assets/js/ds')
...
console.log(`  ${cssFiles} stylesheets, fonts → assets/fonts, sprite → assets/icons, runtime → assets/js/ds`)
